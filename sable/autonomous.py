...
class AccessTracker:
    ...
    def record(self, table: str, columns: Iterable[str] = (), operation: str = "query",
               access_freq: float = 1.0, timestamp: float | None = None):
        ts = time.time() if timestamp is None else timestamp
        entry = {"table": table, "columns": tuple(columns), "operation": operation,
                 "access_freq": access_freq, "timestamp": ts}
        ...
        if access_freq > 0.5: self.tier_counts["hot"] += 1
        elif access_freq > 0.1: self.tier_counts["warm"] += 1
        else: self.tier_counts["cold"] += 1

    def access_frequency(self, table: str, column: str | None = None) -> float:
        entries = self.column_access.get(column, []) if column else self.table_access.get(table, [])
    ...
    def hot_columns(self, table: str, top_n: int = 3) -> list[str]:
        cols = [c for c in self.column_access if any(e["table"] == table for e in self.column_access[c])]
        return sorted(cols, key=lambda c: self.access_frequency(table, c), reverse=True)[:top_n]

    def tier_for_access(self, access_freq: float) -> str:
        return "hot" if access_freq > 0.5 else ("warm" if access_freq > 0.1 else "cold")

    def status(self) -> dict[str, Any]:
        return {"window": self.window, "tier_counts": self.tier_counts,
                "tracked_tables": len(self.table_access), "tracked_columns": len(self.column_access)}


@dataclass
class AdaptiveStorageManager:
    """Hot/warm/cold tier migration with compression selection."""
    def __init__(self, classifier: Any = None, hot_columns: Iterable[str] = (),
    ...
    def classify(self, access_freqs: list[float], rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        self.tier_stats = {"hot": result.get("hot", 0), "warm": result.get("warm", 0),
                           "cold": result.get("cold", 0)}
        return result

    def migrate(self, tier: str, rows: list[dict[str, Any]]):
        if tier not in ("hot", "warm", "cold"): raise ValueError("tier must be hot, warm, or cold")
        if self.classifier is None:
            from .research import HotWarmColdStore
            self.classifier = HotWarmColdStore(segment_rows=1024)
        if tier == "hot": self.classifier.hot_store.append(rows)
        elif tier == "warm": self.classifier.warm_store.append(rows)
        else: self.classifier.cold_store.append(rows)
    ...
    def promote(self, tier: str, rows: list[dict[str, Any]]):
        if tier not in ("hot", "warm"): raise ValueError("tier must be hot or warm")
        self.migrate(tier, rows)
        return self.tier_stats

    def demote(self, tier: str, rows: list[dict[str, Any]]):
        if tier not in ("warm", "cold"): raise ValueError("tier must be warm or cold")
        self.migrate(tier, rows)
        return self.tier_stats
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {"mode": mode, "hot_columns": sorted(self.hot_columns),
                "compression": self.compression, "tier_stats": self.tier_stats}
...
class CostModel:
    ...
    def __init__(self, window: int = 1000):
        ...
        self.parameters: dict[str, float] = {"base": 50.0, "per_row": 0.01, "index_bonus": -0.05}

    def observe(self, rows_scanned: int, rows_returned: int, latency_us: int,
                index_used: bool = False, join: bool = False, aggregate: bool = False):
        self.samples.append({"rows_scanned": rows_scanned, "rows_returned": rows_returned,
                             "latency_us": latency_us, "index_used": index_used,
    ...
    def estimate(self, rows_scanned: int, index_used: bool = False, join: bool = False,
                 aggregate: bool = False) -> float:
        cost = self.parameters["base"] + self.parameters["per_row"] * rows_scanned
        if index_used: cost *= 0.2
        if join: cost *= 3.0
        if aggregate: cost *= 1.5
        return max(1.0, cost)
...
class AdaptivePlanner:
    ...
    def select(self, sql: str, params: Iterable[Any] = (), estimated_rows: int = 0,
               indexes: Iterable[str] = (), candidates: Iterable[str] = ("scan",)):
        low = sql.lower()
        has_join = " join " in low
        has_aggregate = any(x in low for x in ("group by", "avg(", "count(", "sum(", "min(", "max("))
        has_where = " where " in low and "=" in low
        plans = list(candidates) or ["scan"]
        if has_join: plans = ["join", "scan"]
        elif has_aggregate: plans = ["aggregate", "scan"]
        elif has_where:
            # A logical index plan is only a valid candidate when the caller
            # reports a matching physical index. SQLite may still use its
            # primary/unique-key indexes; those are verified separately by
            # SableDB.execute().
            available = set(indexes)
            plans = ["point_lookup", "scan"]
            if "idx_sable_key" in available:
                plans.insert(0, "index_lookup")
        self.candidate_plans[sql] = plans
        if estimated_rows <= 0: estimated_rows = 0
        scored = []
        for plan in plans:
            cost = 0.0
            if plan == "scan": cost = self.cost_model.estimate(estimated_rows)
            elif plan == "point_lookup": cost = self.cost_model.estimate(estimated_rows, index_used=False) * 0.5
            elif plan == "index_lookup": cost = self.cost_model.estimate(estimated_rows, index_used=True)
            elif plan == "join": cost = self.cost_model.estimate(estimated_rows, join=True)
            elif plan == "aggregate": cost = self.cost_model.estimate(estimated_rows, aggregate=True)
            elif plan == "write": cost = self.cost_model.estimate(estimated_rows)
            else: cost = self.cost_model.estimate(estimated_rows)
            scored.append((cost, plan))
        self.last_plan = min(scored)[1]
        return self.last_plan, tuple(plans), False

    def observe(self, plan: str, rows_scanned: int, latency_us: int, index_used: bool = False,
                join: bool = False, aggregate: bool = False):
        self.cost_model.observe(rows_scanned, rows_scanned, latency_us, index_used, join, aggregate)
...
class AdaptiveExecutionEngine:
    ...
    def __init__(self, batch_size: int = 65536, max_parallelism: int = 4,
    ...
    def choose_batch_size(self, rows: int, latency_us: int = 0, throughput: float = 1.0) -> int:
        ...
        self.history.append({"batch_size": self.batch_size, "rows": rows, "latency_us": latency_us})
        self.history = self.history[-1000:]
        return self.batch_size

    def choose_parallelism(self, rows: int, cores: int | None = None) -> int:
        cores = cores or max(1, self.max_parallelism)
        self.current_parallelism = min(cores, max(1, int(math.ceil(rows / max(self.batch_size, 1)))))
    ...
    def status(self) -> dict[str, Any]:
        return {"batch_size": self.batch_size, "max_parallelism": self.max_parallelism,
                "current_parallelism": self.current_parallelism,
                "memory_budget_mb": self.memory_budget_mb, "history_len": len(self.history)}


@dataclass
class AutonomousPhysicalDesigner:
    """Continuous validation and auto-apply of physical design decisions."""
    def __init__(self, validator: Any = None, auto_apply: bool = True,
    ...
    def propose(self, typ: str, details: dict[str, Any], reason: str = "") -> dict[str, Any]:
        proposal = {"type": typ, "details": details, "reason": reason,
                    "status": "proposed", "timestamp": time.time()}
        self.proposals.append(proposal)
        return proposal

    def validate(self, proposal: dict[str, Any], baseline: Any, candidate: Any,
        ...
        result = {"proposal_type": proposal["type"], "validated": validated,
                  "winner": winner, "repetitions": reps, "timestamp": time.time()}
    ...
    def apply(self, proposal: dict[str, Any], apply_fn: Any):
        ...
        if not self.auto_apply: return False
    ...
    def auto_apply_validated(self, apply_fn: Any):
        ...
        for p in list(self.proposals):
            if p["status"] == "proposed" and p.get("validation", {}).get("validated"):
                if self.apply(p, apply_fn): applied += 1
        return applied

    def status(self) -> dict[str, Any]:
        return {"auto_apply": self.auto_apply, "validation_repetitions": self.validation_repetitions,
                "proposals": len(self.proposals), "validation_results": len(self.validation_results)}
