...
class SegmentStore:
    ...
    def _save_manifest(self):
        tmp=self.meta_path.with_suffix(".tmp"); tmp.write_text(json.dumps({"version":1,"mode":self.mode,"segment_rows":self.segment_rows,"segments":self.segments},indent=2)); os.replace(tmp,self.meta_path)
    def append(self, rows: Iterable[dict[str,Any]]):
        ...
        for start in range(0,len(batch),self.segment_rows):
            ...
            if self.mode=="row":
                payload=json.dumps(chunk,separators=(",",":"),sort_keys=True).encode(); (self.root/(filename+".rows")).write_bytes(payload)
            else:
                cols={k:[r.get(k) for r in chunk] for k in {k for r in chunk for k in r}}
                (self.root/(filename+".cols")).write_bytes(pickle.dumps(cols,protocol=5)); payload=pickle.dumps(cols,protocol=5)
    ...
    def scan(self, columns: Sequence[str]|None=None, predicate: Callable[[dict[str,Any]],bool]|None=None):
        ...
        for seg in self.segments:
            if self.mode=="row": rows=json.loads((self.root/(seg["file"]+".rows")).read_bytes())
            else:
                cols=pickle.loads((self.root/(seg["file"]+".cols")).read_bytes()); n=seg["rows"]; rows=[{k:cols[k][i] for k in cols} for i in range(n)]
            for r in rows:
    ...
    def migrate(self, mode: str):
        ...
        rows=self.scan(); old=list(self.segments); self.mode=mode; self.segments=[]
        for seg in old:
            for suffix in (".rows",".cols"):
                p=self.root/(seg["file"]+suffix)
                if p.exists():p.unlink()
        self.append(rows); self._save_manifest(); return True
