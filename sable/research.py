...
class SegmentStore:
    ...
    def __init__(self, path: str, mode="row", segment_rows=4096):
        self.root=Path(path); self.root.mkdir(parents=True, exist_ok=True); self.segment_rows=segment_rows
        self.meta_path=self.root/"manifest.json"; self.segments: list[dict[str,Any]]=[]
        if self.meta_path.exists(): self._load_manifest()
        else: self.mode=mode; self._save_manifest()
    def _load_manifest(self):
        meta=json.loads(self.meta_path.read_text()); self.mode=meta["mode"]; self.segment_rows=meta["segment_rows"]; self.segments=meta["segments"]
    def _save_manifest(self):
        tmp=self.meta_path.with_suffix(".tmp")
        tmp.write_text(json.dumps({"version":1,"mode":self.mode,"segment_rows":self.segment_rows,"segments":self.segments},indent=2))
    ...
    def _read_segment(self, seg):
        suffix = ".rows" if self.mode == "row" else ".cols"
        path = self.root / (seg["file"] + suffix)
        payload = path.read_bytes()
        if hashlib.sha256(payload).hexdigest() != seg.get("sha256"):
    ...
    def append(self, rows: Iterable[dict[str,Any]]):
        ...
        for start in range(0,len(batch),self.segment_rows):
            chunk=batch[start:start+self.segment_rows]; sid=len(self.segments); filename=f"segment-{sid:06d}"
            if self.mode=="row":
                payload=json.dumps(chunk,separators=(",",":"),sort_keys=True).encode(); suffix=".rows"
            else:
                cols={k:[r.get(k) for r in chunk] for k in {k for r in chunk for k in r}}
                payload=pickle.dumps(cols,protocol=5); suffix=".cols"
            target = self.root / (filename + suffix); tmp = target.with_suffix(target.suffix + ".tmp")
            tmp.write_bytes(payload)
            with tmp.open("rb") as handle: os.fsync(handle.fileno())
            os.replace(tmp, target)
            self.segments.append({"id":sid,"rows":len(chunk),"file":filename,"sha256":hashlib.sha256(payload).hexdigest()})
        self._save_manifest(); return len(batch)
    def scan(self, columns: Sequence[str]|None=None, predicate: Callable[[dict[str,Any]],bool]|None=None):
        out=[]
        for seg in self.segments:
            if self.mode=="row": rows=json.loads(self._read_segment(seg))
            else:
                cols=pickle.loads(self._read_segment(seg)); n=seg["rows"]; rows=[{k:cols[k][i] for k in cols} for i in range(n)]
            for r in rows:
                if predicate is None or predicate(r): out.append({k:r.get(k) for k in columns} if columns else r)
        return out
    def migrate(self, mode: str):
        if mode not in ("row","column"): raise ValueError("mode must be row or column")
        if mode==self.mode:return False
        rows=self.scan(); old_mode=self.mode; old=list(self.segments)
        staging = self.root / ".migration"
        if staging.exists():
            import shutil; shutil.rmtree(staging)
        candidate = SegmentStore(str(staging), mode, self.segment_rows); candidate.append(rows)
        for seg in old:
        ...
        staging.rmdir()
        self.mode=mode; self.segments=candidate.segments; self._save_manifest(); return True
    def stats(self): return {"mode":self.mode,"segments":len(self.segments),"rows":sum(s["rows"] for s in self.segments),"bytes":sum((self.root/(s["file"]+(".rows" if self.mode=="row" else ".cols"))).stat().st_size for s in self.segments)}

class RowStore:
    mode="row"
    def __init__(self, rows: Iterable[dict[str,Any]]=(), path: str|None=None):
        self.rows=[dict(r) for r in rows]; self.segment=SegmentStore(path,"row") if path else None
        if self.segment and self.rows:self.segment.append(self.rows)
    def append(self,row): self.rows.append(dict(row)); self.segment and self.segment.append([row])
    def scan(self,columns=None): return self.segment.scan(columns) if self.segment else ([{k:r.get(k) for k in columns} for r in self.rows] if columns else list(self.rows))

class ColumnStore:
    mode="column"
    def __init__(self, rows: Iterable[dict[str,Any]]=(), path: str|None=None):
        self.columns={}; self.segment=SegmentStore(path,"column") if path else None
        if self.segment:self.columns={}; self.rows=[]; self.load(rows)
        else:self.load(rows)
    def load(self,rows):
        batch=[dict(r) for r in rows]
        if self.segment:self.segment.append(batch); return
        for row in batch:
            for key in row:self.columns.setdefault(key,[]).append(row[key])
            for key in set(self.columns)-set(row):self.columns[key].append(None)
    def append(self,row):self.load([row])
    def scan(self,columns=None):
        if self.segment:return self.segment.scan(columns)
        cols=columns or list(self.columns); n=len(next(iter(self.columns.values()),[])); return [{k:self.columns.get(k,[None]*n)[i] for k in cols} for i in range(n)]

class HybridStore:
    mode="hybrid"
    def __init__(self,rows=(),hot_columns=(),path: str|None=None):self.hot_columns=set(hot_columns); self.row=RowStore(rows,path); self.column=ColumnStore(self.row.rows)
...
class JoinExecutor:
    ...
    def hash_join(left,right,key):
        index={}
        for b in right:index.setdefault(b.get(key),[]).append(b)
        return [{**a,**{f"r_{k}":v for k,v in b.items()}} for a in left for b in index.get(a.get(key),[])]
    @staticmethod
    def merge_join(left,right,key,presorted=False):
        a=left if presorted else sorted(left,key=lambda x:x.get(key)); b=right if presorted else sorted(right,key=lambda x:x.get(key)); i=j=0; out=[]
        while i<len(a) and j<len(b):
            if a[i].get(key)<b[j].get(key):i+=1
            elif a[i].get(key)>b[j].get(key):j+=1
            else:
                ii=i
                while ii<len(a) and a[ii].get(key)==a[i].get(key):
                    jj=j
                    while jj<len(b) and b[jj].get(key)==b[j].get(key):out.append({**a[ii],**{f"r_{x}":y for x,y in b[jj].items()}}); jj+=1
                    ii+=1
                i=ii
        return out
    @classmethod
    def execute(cls,left,right,key,algorithm="hash",presorted=False):
        return cls.merge_join(left,right,key,presorted) if algorithm=="merge" else {"nested_loop":cls.nested_loop,"hash":cls.hash_join}[algorithm](left,right,key)
    @classmethod
    def choose(cls,left_rows,right_rows,sorted_inputs=False):
        if min(len(left_rows),len(right_rows))<64:return "nested_loop"
...
class CompressionCodec:
    ...
    def encode(values,codec="zlib"):
        if codec=="none":return pickle.dumps(list(values),protocol=5)
        if codec=="delta":
            nums=[int(v) for v in values]; raw=struct.pack(f"<{len(nums)}q",*([nums[0]]+[nums[i]-nums[i-1] for i in range(1,len(nums))])) if nums else b""; return zlib.compress(raw,9)
        if codec=="rle":
            out=[]
            for v in values:
    ...
    def decode(blob,codec="zlib"):
        if codec=="none":return pickle.loads(blob)
        if codec=="delta":
            raw=zlib.decompress(blob); n=len(raw)//8
            if not n:return []
            d=list(struct.unpack(f"<{n}q",raw)); out=[d[0]]
            for x in d[1:]:out.append(out[-1]+x)
            return out
        vals=pickle.loads(zlib.decompress(blob)); return [v for v,n in vals for _ in range(n)] if codec=="rle" else vals
    @classmethod
    def choose(cls,values,access_frequency=1.0,cpu_budget=1.0):
        if not values:return "none"
        sizes={c:len(cls.encode(values,c)) for c in cls.codecs}; return min(sizes,key=sizes.get) if access_frequency<cpu_budget else "none"
    @classmethod
    def profile(cls,values):return {c:{"bytes":len(cls.encode(values,c)),"roundtrip":cls.decode(cls.encode(values,c),c)==list(values)} for c in cls.codecs}

@dataclass
class ShadowResult:
    baseline:str; candidate:str; baseline_us:int; candidate_us:int; winner:str; rows_equal:bool; repetitions:int=1; speedup:float=1.0
class ShadowExecutor:
    @staticmethod
    def compare(baseline,candidate,baseline_name="baseline",candidate_name="candidate",repetitions=3):
        def run(fn):
            vals=[]; result=None
            for _ in range(repetitions):
                t=time.perf_counter_ns(); result=fn(); vals.append((time.perf_counter_ns()-t)//1000)
            return result, sorted(vals)[len(vals)//2]
        a,at=run(baseline); b,bt=run(candidate); winner=candidate_name if bt<at else baseline_name
        return ShadowResult(baseline_name,candidate_name,at,bt,winner,a==b,repetitions,(at/bt if bt else float("inf")))

class UCB1Bandit:
    def __init__(self,arms,seed=7):self.arms=list(arms); self.counts={a:0 for a in self.arms}; self.rewards={a:0.0 for a in self.arms}; self.rng=random.Random(seed)
    def select(self):
        unseen=[a for a in self.arms if not self.counts[a]]
        if unseen:return unseen[0]
        total=sum(self.counts.values()); return max(self.arms,key=lambda a:self.rewards[a]/self.counts[a]+math.sqrt(2*math.log(total)/self.counts[a]))
    def update(self,arm,reward):self.counts[arm]+=1; self.rewards[arm]+=reward
    def status(self):return {a:{"trials":self.counts[a],"mean_reward":self.rewards[a]/self.counts[a] if self.counts[a] else 0} for a in self.arms}

class TabularQLearner:
    def __init__(self,actions,alpha=.2,gamma=.9,epsilon=.1,seed=7):self.actions=list(actions); self.q={}; self.alpha=alpha; self.gamma=gamma; self.epsilon=epsilon; self.rng=random.Random(seed)
    def _row(self,state):return self.q.setdefault(tuple(state),{a:0.0 for a in self.actions})
    def choose(self,state,explore=True):
        row=self._row(state)
        if explore and self.rng.random()<self.epsilon:return self.rng.choice(self.actions)
