...
class SegmentStore:
    ...
    def append(self, rows: Iterable[dict[str, Any]]):
        ...
        for start in range(0, len(batch), self.segment_rows):
            ...
            if self.mode == "row":
                ...
                suffix = ".cols"
            target = self.root / (filename + suffix)
    ...
    def scan(
        ...
        for seg in self.segments:
            if self.mode == "row":
                rows = json.loads(self._read_segment(seg))
            else:
                cols = pickle.loads(self._read_segment(seg))
                n = seg["rows"]
    ...
    def migrate(self, mode: str):
        ...
        candidate = SegmentStore(str(staging), mode, self.segment_rows)
