...
class WorkloadModel:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "join": self.join,
            "write": self.write,
            "latencies": self.latencies[-10000:],
...
class AdaptiveEngine:
    ...
    def state_dict(self) -> dict[str, Any]:
        return {
            ...
            "cost_model": {
                "samples": self.cost_model.samples[-1000:],
                "parameters": self.cost_model.parameters,
            },
            "join_bandit": {
                "counts": self.join_bandit.counts,
                "rewards": self.join_bandit.rewards,
            },
            "q_states": [
                [list(state), values] for state, values in self.policy_learner.q.items()
            ],
            "execution": self.execution_engine.status(),
            "access_tracker": self.access_tracker.status(),
        }
    ...
    def restore_state(self, state: dict[str, Any]) -> None:
        ...
        self.layout = state.get("layout", "row")
        self.compression = state.get("compression", "none")
        self.prefetch = state.get("prefetch", "adaptive")
        self.buffer_policy = state.get("buffer_policy", "workload-aware")
        if state.get("schema"):
            self.schema = SchemaMetadata.from_dict(state["schema"])
        self._query_id = state.get("query_id", len(self.telemetry))
        self._last_phase = state.get("last_phase", "startup")
        self.policy_mode = state.get("policy_mode", "heuristic")
        if state.get("model"):
            self.model = WorkloadModel.from_dict(state["model"])
        ...
        self.join_bandit.counts.update(bandit.get("counts", {}))
        self.join_bandit.rewards.update(bandit.get("rewards", {}))
        self.policy_learner.q = {
            tuple(item[0]): item[1] for item in state.get("q_states", [])
        }
...
class SableDB:
    def __init__(self, path: str = ":memory:", epoch_size: int = 100):
        ...
        self.conn.execute(
            "CREATE TABLE IF NOT EXISTS _sable_state (state_key TEXT PRIMARY KEY, state_json TEXT NOT NULL, updated_at REAL NOT NULL)"
        )
        self.conn.commit()
    ...
    def _load_state(self) -> None:
        row = self.conn.execute(
            "SELECT state_json FROM _sable_state WHERE state_key='adaptive'"
        ).fetchone()
        if row:
            try:
                self.adaptive.restore_state(json.loads(row[0]))
            except (TypeError, ValueError, json.JSONDecodeError):
                self.adaptive = AdaptiveEngine(self.conn, self.adaptive.epoch_size)
        if self._has_index("idx_sable_key"):
            self.adaptive.indexes.add("idx_sable_key")
        else:
            self.adaptive.indexes.discard("idx_sable_key")

    def _persist_state(self) -> None:
        if self.conn.in_transaction:
            return
        payload = json.dumps(
            self.adaptive.state_dict(), default=str, separators=(",", ":")
        )
        self.conn.execute(
            "INSERT INTO _sable_state(state_key,state_json,updated_at) VALUES('adaptive',?,?) "
    ...
    def execute(self, sql: str, params: Iterable[Any] = ()) -> list[dict[str, Any]]:
        ...
        index_used = any(
            "USING INDEX" in detail.upper()
            or "USING INTEGER PRIMARY KEY" in detail.upper()
        ...
        self.adaptive.record(
            Telemetry(
                ...
                (
                    "idx_sable_key"
                    if index_used and "idx_sable_key" in " ".join(sqlite_plan)
                    else ""
                ),
            )
        )
