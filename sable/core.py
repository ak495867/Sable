...
from .research import (CompressionCodec, JoinExecutor, SegmentStore,
                        ShadowExecutor, TabularQLearner, UCB1Bandit,
                        VectorizedExecutor, HotWarmColdStore)
...
class SchemaMetadata:
    ...
    def add_column(self, name: str, dtype: str, distinct: int = 0, nulls: int = 0,
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"table_name": self.table_name, "version": self.version,
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SchemaMetadata":
        ...
        return cls(table_name=data["table_name"], columns=cols,
...
class WorkloadModel:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"window": self.window, "total": self.total, "point": self.point,
                "scan": self.scan, "aggregate": self.aggregate, "join": self.join,
                "write": self.write, "latencies": self.latencies[-10000:],
                "recent": self.recent[-self.window:], "fingerprints": self.fingerprints,
                "plan_counts": self.plan_counts, "plan_latency": self.plan_latency,
                "phase_transitions": self.phase_transitions}
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "WorkloadModel":
        model = cls(data.get("window", 100))
        for key in ("total", "point", "scan", "aggregate", "join", "write"):
            setattr(model, key, data.get(key, 0))
        model.latencies = data.get("latencies", [])
        model.recent = data.get("recent", [])
        model.fingerprints = data.get("fingerprints", {})
        model.plan_counts = data.get("plan_counts", {})
        model.plan_latency = data.get("plan_latency", {})
        model.phase_transitions = data.get("phase_transitions", [])
        return model
...
class BoundedUCB1Bandit(UCB1Bandit):
    ...
    def select(self):
        ...
        selected = max(self.arms, key=lambda a: self.rewards[a]/self.counts[a]
                       + min(self.exploration_cap * (2*math.log(total)/self.counts[a])**0.5, self.exploration_cap))
        return selected
    def decay(self):
        for arm in self.arms:
            self.rewards[arm] *= self.decay_rate
    def confidence_interval(self, arm: str, confidence: float = 0.95) -> tuple[float, float]:
        if self.counts[arm] < 2: return (0.0, float("inf"))
        mean = self.rewards[arm] / self.counts[arm]
        mean_abs = abs(self.rewards[arm] / self.counts[arm])
        se = mean_abs ** 0.5 / max(self.counts[arm] ** 0.5, 1)
        z = 1.96
...
class AdaptiveEngine:
    ...
    def __init__(self, conn: sqlite3.Connection, epoch_size: int = 100):
        ...
        self.layout = "row"; self.compression = "none"; self.prefetch = "adaptive"
        self.buffer_policy = "workload-aware"; self.schema = SchemaMetadata("sable_records")
        ...
        self.shadow_enabled = True; self.policy_mode = "heuristic"
        ...
        self.physical_designer = AutonomousPhysicalDesigner(auto_apply=False)
    @staticmethod
    ...
    def update_column_stats(self, column: str, values: list[Any]):
        self.schema.row_count = max(self.schema.row_count, len(values))
        ...
        self.schema.add_column(column, "NUMERIC" if numeric else "TEXT",
    ...
    def estimate_index_benefit(self, column: str) -> dict[str, Any]:
        ...
        if not stat: return {"benefit_score": 0.0, "estimated_reduction_pct": 0.0}
        selectivity = stat.distinct_count / max(self.schema.row_count, 1)
        benefit = min(1.0, selectivity * 10)
        reduction = min(95.0, benefit * 100)
        result = {"column": column, "benefit_score": round(benefit, 4),
                  "estimated_reduction_pct": round(reduction, 2),
                  "distinct_count": stat.distinct_count, "cardinality_ratio": stat.cardinality_ratio}
    ...
    def state_dict(self) -> dict[str, Any]:
        return {"epoch": self.epoch, "plan_cache": self.plan_cache,
                "telemetry": [asdict(t) for t in self.telemetry[-5000:]],
                "proposals": self.proposals[-500:], "indexes": sorted(self.indexes),
                "layout": self.layout, "compression": self.compression,
                "prefetch": self.prefetch, "buffer_policy": self.buffer_policy,
                "schema": self.schema.to_dict(), "query_id": self._query_id,
                "last_phase": self._last_phase, "policy_mode": self.policy_mode,
                "model": self.model.to_dict(), "index_benefits": self.index_benefits,
                "compression_candidates": self.compression_candidates,
                "adaptation_journal": self.adaptation_journal[-1000:],
                "rollback_history": self.rollback_history[-1000:],
                "proposal_validation_results": self._proposal_validation_results[-500:],
                "cost_model": {"samples": self.cost_model.samples[-1000:], "parameters": self.cost_model.parameters},
                "join_bandit": {"counts": self.join_bandit.counts, "rewards": self.join_bandit.rewards},
                "q_states": [[list(state), values] for state, values in self.policy_learner.q.items()],
                "execution": self.execution_engine.status(), "access_tracker": self.access_tracker.status()}

    def restore_state(self, state: dict[str, Any]) -> None:
        self.epoch = state.get("epoch", 0)
        self.plan_cache = state.get("plan_cache", {})
        self.telemetry = [Telemetry(**t) for t in state.get("telemetry", [])]
        self.proposals = state.get("proposals", [])
        self.indexes = set(state.get("indexes", []))
        self.layout = state.get("layout", "row"); self.compression = state.get("compression", "none")
        self.prefetch = state.get("prefetch", "adaptive"); self.buffer_policy = state.get("buffer_policy", "workload-aware")
        if state.get("schema"): self.schema = SchemaMetadata.from_dict(state["schema"])
        self._query_id = state.get("query_id", len(self.telemetry)); self._last_phase = state.get("last_phase", "startup")
        self.policy_mode = state.get("policy_mode", "heuristic")
        if state.get("model"): self.model = WorkloadModel.from_dict(state["model"])
        self.index_benefits = state.get("index_benefits", {})
        self.compression_candidates = state.get("compression_candidates", {})
        self.adaptation_journal = state.get("adaptation_journal", [])
        self.rollback_history = state.get("rollback_history", [])
        self._proposal_validation_results = state.get("proposal_validation_results", [])
        cost = state.get("cost_model", {})
        self.cost_model.samples = cost.get("samples", [])[-self.cost_model.window:]
        self.cost_model.parameters.update(cost.get("parameters", {}))
        bandit = state.get("join_bandit", {})
        self.join_bandit.counts.update(bandit.get("counts", {})); self.join_bandit.rewards.update(bandit.get("rewards", {}))
        self.policy_learner.q = {tuple(item[0]): item[1] for item in state.get("q_states", [])}
    
    def record(self, telemetry: Telemetry):
        self.telemetry.append(telemetry)
        self.model.observe(telemetry.fingerprint, telemetry.plan, telemetry.latency_us,
                           telemetry.rows_scanned, telemetry.rows_returned, telemetry.operation)
        phase = self.model.phase()
        if phase != self._last_phase and self.model.total > self.epoch_size:
            self.proposals.append({"type":"phase_change", "from":self._last_phase, "to":phase,
                                   "status":"observed", "epoch":self.epoch})
        self._last_phase = phase
        if self.model.total % self.epoch_size == 0: self.adapt()
    def adapt(self):
        self.epoch += 1; s = self.model.snapshot()
        recent_point = s["recent_point_lookup_ratio"]; recent_scan = s["recent_scan_ratio"]
        if recent_point > .60 and "idx_sable_key" not in self.indexes:
            benefit = self.estimate_index_benefit("key")
            self._propose_once("create_index", "recent point lookup ratio exceeded 60%",
                               {"index":"idx_sable_key", "benefit":benefit["estimated_reduction_pct"]},
                               benefit=benefit)
        if recent_scan > .70 and self.layout != "column":
            self._propose_once("layout", "recent scan ratio exceeded 70%",
                               {"target":"column", "adaptation_cost":"segment rewrite"})
        if s["write_ratio"] > .50 and self.compression != "none":
            self._propose_once("compression", "write ratio exceeded 50%",
        ...
        if recent_scan > .50 and self.storage_classifier is not None:
            self._propose_once("hot_warm_cold", "scan-heavy workload detected",
    ...
    def _propose_once(self, typ: str, reason: str, details: dict[str, Any], **extra):
        ...
        proposal = {"type":typ, "reason":reason, "details":details, "status":"proposed",
                    "epoch":self.epoch, "timestamp":time.time(), **extra}
    ...
    def _validate_with_shadow(self, proposal: dict[str, Any]) -> dict[str, Any]:
        baseline_latency = statistics.median(self.model.latencies[-100:]) if self.model.latencies else 0
        result = {"baseline_latency_us": baseline_latency,
                  "estimated_improvement": proposal["details"].get("benefit", 0),
                  "validated": True, "confidence": "high" if baseline_latency > 0 else "low"}
    ...
    def approve_proposal(self, n: int = -1):
        ...
        try:
            if p["type"] == "create_index":
                self.conn.execute("CREATE INDEX IF NOT EXISTS idx_sable_key ON sable_records(key)")
                self.conn.commit(); self.indexes.add("idx_sable_key")
                benefit = self.estimate_index_benefit("key")
            ...
            p["status"] = "committed"; p["committed_at"] = time.time()
    ...
    def reject_proposal(self, n: int = -1):
        ...
        p["status"] = "rejected"; p["rejected_at"] = time.time(); return p
    def rollback_last_commit(self):
        if not self.adaptation_journal: return None
        last = None
        ...
        if p and p["type"] == "create_index":
            try:
                ...
                self.conn.commit(); self.indexes.discard("idx_sable_key")
                p["status"] = "rolled_back"
            except Exception: pass
        last["action"] = "rolled_back"
        return last
    def _append_journal(self, proposal: dict[str, Any], action: str):
        self.adaptation_journal.append({"timestamp":time.time(), "epoch":self.epoch,
            "proposal_type":proposal["type"], "action":action,
            "proposal_id":id(proposal), "details":proposal["details"]})
    def _append_rollback(self, proposal: dict[str, Any], reason: str):
        proposal_type = proposal.get("type", proposal.get("proposal_type"))
        self.rollback_history.append({"timestamp":time.time(), "epoch":self.epoch,
            "proposal_type":proposal_type, "reason":reason,
            "original_details":proposal["details"], "rollback_type":"revert"})
    ...
    def choose_join_algorithm(self, left_rows, right_rows, key, sorted_inputs=False):
        ...
        if self.policy_mode == "q_learning":
            return self.policy_learner.choose(("join", len(left_rows)//1000, len(right_rows)//1000), explore=False)
    ...
    def recommend_compression(self, values: list[Any]) -> dict[str, Any]:
        ...
        result = {"recommended_codec": best, "savings_bytes": profiles[best]["bytes"],
                  "profiles": profiles, "access_pattern": self.model.phase()}
    ...
    def classify_storage(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        hot = [r for r in rows if r.get("access_freq", 0) > 0.5] if any("access_freq" in r for r in rows) else rows[:max(len(rows)//3, 1)]
        warm = rows[max(len(rows)//3, 1):max(2*len(rows)//3, 1)] if len(rows) > 3 else []
        cold = rows[max(2*len(rows)//3, 1):] if len(rows) > 3 else []
        result = {"hot_count": len(hot), "warm_count": len(warm), "cold_count": len(cold),
                  "classification": "complete"}
        self.storage_classifier._classify(hot, warm, cold)
        return result
    def status(self):
        return {"version":"3.0", "epoch":self.epoch, "policy":self.policy_mode,
                "physical":{"layout":self.layout, "compression":self.compression,
                            "prefetch":self.prefetch, "buffer_policy":self.buffer_policy,
                            "storage_classification": self.storage_classifier.mode if self.storage_classifier else "disabled"},
                "research":{"storage_modes":["row","column","hybrid"], "vector_backend":self.vectorized.backend,
                            "join_algorithms":["nested_loop","hash","merge"],
                            "compression_codecs":list(CompressionCodec.codecs),
                            "shadow_enabled":self.shadow_enabled,
                            "bandit":self.join_bandit.status(), "q_states":len(self.policy_learner.q)},
                "workload":self.model.snapshot(), "proposals":self.proposals[-20:],
                "active_indexes":sorted(self.indexes), "telemetry_events":len(self.telemetry),
                "schema":self.schema.to_dict(), "index_benefits":self.index_benefits,
                "compression_candidates":self.compression_candidates,
...
class SableDB:
    def __init__(self, path: str = ":memory:", epoch_size: int = 100):
        ...
        self.conn.execute("CREATE TABLE IF NOT EXISTS _sable_state (state_key TEXT PRIMARY KEY, state_json TEXT NOT NULL, updated_at REAL NOT NULL)")
    ...
    def _load_state(self) -> None:
        row = self.conn.execute("SELECT state_json FROM _sable_state WHERE state_key='adaptive'").fetchone()
        if row:
            try: self.adaptive.restore_state(json.loads(row[0]))
            except (TypeError, ValueError, json.JSONDecodeError):
                self.adaptive = AdaptiveEngine(self.conn, self.adaptive.epoch_size)
        if self._has_index("idx_sable_key"): self.adaptive.indexes.add("idx_sable_key")
        else: self.adaptive.indexes.discard("idx_sable_key")
    def _persist_state(self) -> None:
        if self.conn.in_transaction:
            return
        payload = json.dumps(self.adaptive.state_dict(), default=str, separators=(",", ":"))
        self.conn.execute("INSERT INTO _sable_state(state_key,state_json,updated_at) VALUES('adaptive',?,?) "
                          "ON CONFLICT(state_key) DO UPDATE SET state_json=excluded.state_json, updated_at=excluded.updated_at",
                          (payload, time.time()))
        self.conn.commit()
    def _sqlite_plan(self, sql: str, params: tuple[Any, ...]) -> tuple[str, ...]:
        if not sql.lstrip().lower().startswith(("select", "with", "pragma")): return ()
        try:
            return tuple(str(row[3]) for row in self.conn.execute("EXPLAIN QUERY PLAN " + sql, params).fetchall())
    ...
    def execute(self, sql: str, params: Iterable[Any] = ()) -> list[dict[str, Any]]:
        ...
        if hasattr(self.adaptive, 'planner'):
            plan, candidates, cache_hit = self.adaptive.planner.select(sql, params, estimated_rows=rows_scanned,
                                                                       indexes=self.adaptive.indexes)
        else:
            plan, candidates, cache_hit = self.adaptive.choose_plan(sql, params)
            
        sqlite_plan = self._sqlite_plan(sql, params)
        ...
        index_used = any("USING INDEX" in detail.upper() or "USING INTEGER PRIMARY KEY" in detail.upper()
                         for detail in sqlite_plan)
        self.adaptive.record(Telemetry(self.adaptive._query_id, self.adaptive.fingerprint(sql, params), plan,
                                       scanned, len(rows), elapsed, index_used,
                                       self.adaptive.model.phase(), self.adaptive.epoch, operation, cache_hit,
                                       len(str(rows).encode()), 0, elapsed, candidates, 0, plan,
                                       sqlite_plan, "idx_sable_key" if index_used and "idx_sable_key" in " ".join(sqlite_plan) else ""))
        
        # NEW v3.0: use execution engine for adaptive optimization
        if hasattr(self.adaptive, 'execution_engine') and rows:
            batch_size = self.adaptive.execution_engine.choose_batch_size(len(rows), elapsed, 1.0)
        ...
        if hasattr(self.adaptive, 'planner'):
            self.adaptive.planner.observe(plan, scanned, elapsed, plan == "index_lookup")
        ...
        if hasattr(self.adaptive, 'access_tracker'):
            ...
            self.adaptive.access_tracker.record(self.adaptive.schema.table_name, columns, operation, access_freq=1.0)
        self._persist_state()
        return rows
    ...
    def close(self):
        if self.conn is not None:
            self._persist_state(); self.conn.close(); self.conn = None
    ...
    def adaptive_status(self): return self.adaptive.status()
    def access_tracker_status(self): return self.adaptive.access_tracker.status()
    def storage_manager_status(self): return self.adaptive.storage_manager.status()
    def cost_model_status(self): return self.adaptive.cost_model.status()
    def execution_status(self): return self.adaptive.execution_engine.status()
    def physical_designer_status(self): return self.adaptive.physical_designer.status()
    ...
    def register_schema(self, columns: list[tuple[str, str]]): self.adaptive.register_schema("sable_records", columns)
    def update_stats(self, column: str, values: list[Any]): self.adaptive.update_column_stats(column, values)
    def get_index_benefit(self, column: str) -> dict[str, Any]: return self.adaptive.estimate_index_benefit(column)
    def get_compression_recommendation(self, values: list[Any]) -> dict[str, Any]: return self.adaptive.recommend_compression(values)
    def classify_storage(self, rows: list[dict[str, Any]]) -> dict[str, Any]: return self.adaptive.classify_storage(rows)
    def rollback_last_adaptation(self): return self.adaptive.rollback_last_commit()
