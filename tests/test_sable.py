...
from sable.autonomous import AccessTracker, AdaptiveStorageManager, CostModel, AdaptivePlanner, AdaptiveExecutionEngine, AutonomousPhysicalDesigner
...
class SableTests(unittest.TestCase):
    ...
    def test_schema_and_column_stats(self):
        db=SableDB(":memory:")
        # Register schema
        db.register_schema([("price", "REAL"), ("symbol", "TEXT"), ("volume", "INTEGER")])
    ...
    def test_rollback_last_adaptation(self):
        ...
        for i in range(20): db.get(f"k{i}")
    ...
    def test_adaptive_planner(self):
        db=SableDB(":memory:")
        # Test plan selection
        plan, candidates, cache_hit = db.adaptive.planner.select("SELECT * FROM sable_records WHERE key = 'test'", ())
    ...
    def test_adaptive_execution_engine(self):
        ...
        batch_size = db.adaptive.execution_engine.choose_batch_size(1000, 50000)  # High latency -> increase batch
        ...
        memory = db.adaptive.execution_engine.choose_memory(1000, 100)  # 1000 rows * 100 bytes/row
        self.assertGreaterEqual(memory, 0)
        self.assertLessEqual(memory, db.adaptive.execution_engine.memory_budget_mb * 1024 * 1024)
    ...
    def test_autonomous_physical_designer(self):
        ...
        proposal = db.adaptive.physical_designer.propose("test_type", {"param": "value"}, "test reason")
        ...
        validation_result = db.adaptive.physical_designer.validate(proposal, None, None, repetitions=1)
    ...
    def test_planner_does_not_claim_missing_index(self):
        db = SableDB(":memory:")
        db.put("a", {"v": 1})
        db.get("a")
        event = db.adaptive.telemetry[-1]
        self.assertNotEqual(event.logical_plan, "")
        self.assertNotIn("index_lookup", event.candidate_plans)
        self.assertNotEqual(event.sqlite_plan, ())
        self.assertEqual(event.index_name, "")
        db.close()

    def test_adaptive_state_persists(self):
        fd, path = tempfile.mkstemp(); os.close(fd)
        try:
            db = SableDB(path); db.put("a", {"v": 1}); db.get("a"); db.close()
            reopened = SableDB(path)
            self.assertEqual(reopened.adaptive.status()["telemetry_events"], 2)
            self.assertEqual(reopened.get("a"), {"v": 1})
            reopened.close()
        finally:
            os.unlink(path)

    def test_cli_json_value_is_not_double_encoded(self):
        fd, path = tempfile.mkstemp(); os.close(fd)
        try:
            subprocess.run([sys.executable, "-m", "sable.cli", path, "put", "demo", "--json", '{"x":1}'], check=True, capture_output=True)
            db = SableDB(path)
            self.assertEqual(db.get("demo"), {"x": 1})
            db.close()
        finally:
            os.unlink(path)
